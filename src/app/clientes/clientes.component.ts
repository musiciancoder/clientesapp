...
export class ClientesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.clienteService.getClientes().pipe(
      ...
      tap(clientes => { //el tap no retorna nada, es un void

        console.log('ClienteService:tap3: ');
        clientes.forEach(cliente => { //se usa forEach con tap, no map
          console.log(cliente.nombre);
        });
      })

    ).subscribe(clientes => this.clientes = clientes);// this.clienteService.getClientes() es la ejecucion de inyeccion por dependencias
